...
Crawler.prototype.depthAllowed = function(queueItem) {
	...
	return (
		crawler.maxDepth === 0 ||
		(queueItem.depth + 1) <= crawler.maxDepth || 
...
Crawler.prototype.discoverResources = function(resourceData,queueItem) {
    ...
    $('a').each(function(){ uris.push($(this).attr('href')); });
    $('img').each(function(){ uris.push($(this).attr('src')); });
    $('link[rel=stylesheet]').each(function(){ uris.push($(this).attr('href')); });
    
    if (crawler.parseScriptTags) {
        $('script').each(function(){ uris.push($(this).attr('src')); });
    }

    var urisobj = {}, i, j, u;
    if (uris){
        for(i = 0, j = uris.length; i < j; i++){
            u = cleanURL(uris[i]);
            try {
                u = URI(u)
            ...
            if (!crawler.protocolSupported(u)) continue;
            if (u in urisobj) continue;
            urisobj[u] = 1; 
        }
    }
    return Object.keys(urisobj);
};
...
Crawler.prototype.queueURL = function(url,queueItem) {
	...
	if (! crawler.domainValid(parsedURL.host)) return false;
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	function processReceivedData() {
		...
		// Are the item's items allowed by depth conditions ?
